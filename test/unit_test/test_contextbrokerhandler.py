...
class TestContextBrokerHandler(TestCase):
    """ Unit test for the ContextBrokerHandler class """
    def setUp(self):
    ...
    def test_request_with_timeout_exception(self):
        self.assertRaises(requests.exceptions.Timeout, self.context_broker_handler._request,
                          "POST", "url", headers=TEST_HEADER)
    ...
    def test_request_with_request_exception(self):
        self.assertRaises(requests.exceptions.RequestException, self.context_broker_handler._request,
                          "POST", "url", headers=TEST_HEADER)
    ...
    def test_subscribe_to_entity_with_error(self):
        self.assertRaises(BrokerException, self.context_broker_handler.subscribe_to_entity,
                          "", [{"id": "id", "type": "type"}], None)
